...
const TransactionFooter = ({ productos, transaction }: Props) => {
  return (
    <div className="h-48 max-sm:h-72 bg-white border border-gray-300 rounded-lg shadow-xl p-4 pt-2 flex flex-col gap-2">
      <div className="flex gap-4 flex-1 overflow-auto max-sm:flex-col">
        <ul
          className={twMerge(
            "flex-1 border rounded-lg h-full flex flex-col overflow-y-auto overflow-x-hidden p-2 gap-2 bg-bg",
            transaction.length === 0 && "items-center justify-center"
          )}
        >
